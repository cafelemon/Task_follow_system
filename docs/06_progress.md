...
当前状态：4.5.0 已完成部门负责人和总经办会议工作台分版本规划；生产通知保持暂停口径。
...
4.4.0 待归类事项责任人处理线规划实施内容：

- 版本推进到 `4.4.0`。
- 本版只做落地规划，不改业务代码、不新增数据库结构、不制作生产离线包。
- 确定 4.4.x 采用“能力主线 + 角色验收”，不再按单一角色硬拆；工作台继续按多身份叠加展示模块。
- 明确 4.4.x 目标是待归类事项责任人处理闭环，不是单纯任务负责人工作台。
- 拆分 4.4.1 至 4.4.6：责任人处理底座、挂载已有部门任务处理、本部门常态化处理、跨部门协作双确认、缺部门任务/缺母任务诉求上提、组合工作台复核。
- 明确 4.5.x 进入部门负责人和总经办会议工作台深化，4.6.x 进入管理员运营工作台深化。
- 明确处理接口后续按动作设计，避免前端直接改状态；待归类事项未转成正式任务前不进入会议看板正式任务统计。
- 生产通知继续按当前暂停记录处理，本线不得顺手恢复通知。

4.4.1 责任人处理底座实施内容：

- 版本推进到 `4.4.1`。
- 新增 `POST /api/work-items/{id}/approve`、`POST /api/work-items/{id}/reject`、`POST /api/work-items/{id}/close`。
- 待归类事项状态扩展使用 `approved`、`rejected` 和 `closed`；非 `pending` 事项重复处理返回冲突。
- 退回必须填写退回原因，关闭必须填写关闭说明；所有动作写入 `work_item_events`，事项序列化返回事件记录和可操作权限。
- 工作台“待我确认”取消只读提示，按权限显示同意、退回和关闭；提交人可在“我的提交”查看最近处理记录。
- 跨部门协作事项本版不开放同意，避免绕过 4.4.4 的双确认，只允许退回或关闭。
- 周报草稿只聚合 `pending` 和 `approved` 待归类事项，过滤已撤回、已退回和已关闭事项；历史周报快照不自动改写。
- 本版不做转正式子任务、缺部门任务/缺母任务上提、跨部门双确认、通知恢复或生产离线包。

4.4.2 挂载已有部门任务处理实施内容：

- 版本推进到 `4.4.2`。
- 新增 `converted_to_sub_task` 待归类事项状态，runtime schema 增加 `converted_sub_task_id` 和 `converted_at`。
- 新增 `POST /api/work-items/{id}/convert-to-sub-task`，仅挂载已有部门任务事项可调用，允许 `pending` 和 `approved` 状态转换。
- 转换时创建正式子任务，负责人继承关联部门任务负责人，执行人由处理人选择，截止日期可空，并写入 `TaskEvent` 和 `WorkItemEvent`。
- 工作台“待我处理”中对可转换事项显示“转子任务”按钮，弹窗支持编辑标题、选择执行人、填写截止日期和转换说明。
- 转换成功后事项展示关联子任务摘要，已转子任务事项不再进入待归类周报草稿，后续通过正式子任务周更新进入周报。
- 本版不做本部门常态化深化、跨部门双确认、缺部门任务/缺母任务上提、通知恢复或生产离线包。

4.4.3 本部门常态化工作处理实施内容：

- 版本推进到 `4.4.3`。
- 新增 `GET /api/work-items?scope=department-routine`，部门负责人查看本人所属部门常态化事项记录，管理员和权限管理人员可兜底查看。
- 工作台“待归类事项”增加“本部门常态化”视图，展示提交人、提交部门、状态、最近处理事件和处理说明。
- 待确认常态化事项继续复用 4.4.1 的同意、退回和关闭动作；已确认、已退回、已关闭事项只读追溯。
- 周报草稿继续聚合本人提交且状态为 `pending` 或 `approved` 的常态化事项；已退回和已关闭事项不进入周报草稿。
- 本版不做跨部门双确认、缺部门任务/缺母任务上提、通知恢复或生产离线包。

4.4.4 跨部门协作双确认实施内容：

- 版本推进到 `4.4.4`。
- `work_items` runtime schema 增加提交部门确认和协作部门确认字段，保存确认人、确认时间和确认说明。
- 新增 `POST /api/work-items/{id}/cross-department-approve`，仅跨部门协作事项可用；两侧均确认后状态才变为 `approved`。
- 工作台跨部门事项卡片展示双确认进度，当前用户只看到自己有权确认的一侧；管理员和权限管理人员可兜底指定确认侧。
- 任一方退回或关闭后流程结束，事项不进入后续周报草稿；已确认事项进入提交人个人周报材料和协作记录。
- 周报草稿过滤未完成双确认的跨部门事项，只有状态为 `approved` 的跨部门事项进入周报材料。
- 本版不做缺部门任务/缺母任务上提、通知恢复或生产离线包。

4.4.5 缺部门任务/缺母任务诉求上提实施内容：

- 版本推进到 `4.4.5`。
- 新增 `escalated_department_task` 和 `escalated_parent_task` 状态，runtime schema 增加上提人、上提时间和上提说明字段。
- 新增 `POST /api/work-items/{id}/escalate`，责任人可将 `pending` 或 `approved` 事项上提为缺部门任务或缺母任务诉求。
- 工作台新增“缺部门任务诉求”和“缺母任务诉求”视图；部门负责人查看本人部门诉求，总经办/秘书查看缺母任务诉求，管理员和权限管理人员兜底。
- 上提诉求不自动创建部门任务或母任务，不进入正式任务统计，也不进入后续周报草稿。
- 本版不做诉求转任务、会议决议落地、通知恢复或生产离线包。

4.4.6 组合工作台复核与 4.5 交接实施内容：

- 版本推进到 `4.4.6`。
- 工作台主文案从单一执行者入口收口为个人行动入口，待归类事项区按多角色叠加展示“我的提交 / 待我处理 / 本部门常态化 / 缺部门任务诉求 / 缺母任务诉求”。
- 待归类事项卡片优化长文本换行、可选处理字段展示、已转子任务提示和已上提诉求提示，减少空字段噪音。
- “待我处理”明确只展示当前可处理事项；缺部门任务和缺母任务诉求视图明确进入 4.5.x 深化处理，本版只做追溯展示。
- 工作台、待归类事项和周报中心移动端继续使用全宽按钮和卡片式空状态，保证手机端不出现全局横向滚动。
- 文档同步记录：4.4.x 已完成责任人处理闭环基础能力，4.5.x 进入部门负责人和总经办会议工作台深化；不安排 4.4.7 至 4.4.9。
- 本版不新增数据库结构、不恢复生产通知、不制作生产离线包。

4.5.0 部门负责人和总经办会议工作台分版本规划实施内容：

- 版本推进到 `4.5.0`。
- 本版只做规划和文档同步，不改业务代码、不新增数据库结构、不制作生产离线包。
- 明确 4.5.x 目标是部门负责人和总经办会议工作台深化，承接 4.4.x 的缺部门任务诉求、缺母任务诉求和责任人处理闭环。
- 拆分 4.5.1 至 4.5.6：部门负责人工作台框架、缺部门任务诉求处理、总经办/秘书会议工作台框架、缺母任务诉求处理、会议处理流程与会后跟进、4.5 收尾与 4.6 交接。
- 明确缺部门任务诉求必须在现有母任务下新建部门任务，找不到合适母任务时转为缺母任务诉求；缺母任务诉求由总经办/秘书确认后才能新建母任务。
- 明确 4.5.x 不恢复生产通知、不做飞书汇报原生回写、不做正式会议纪要系统、不进入管理员运营工作台深度能力。
