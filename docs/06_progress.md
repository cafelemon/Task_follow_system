...
当前状态：1.4.0 部门任务编辑与子任务状态修订已完成并通过本地验证，聚焦部门级任务维护、拆解入口、子任务开启/完成和更新页空白修复。
...
- 会议材料导出的正式格式。
- 1.4.0 完成后需继续确认部门任务归档后历史查询和恢复入口是否需要补充。
...
- 完成 1.4.0 后做一次桌面宽屏和窄屏页面检查，重点看母任务详情、部门任务总览、子任务执行和子任务更新页。
