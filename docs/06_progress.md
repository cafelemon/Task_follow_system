...
当前状态：5.0.0 生产同步离线包已生成；本包包含当前 PostgreSQL 数据库快照，用于同步 4.8.x 历史周更新补齐后的数据。

记录日期：2026-06-16。

生产运行备注：5.0.0 起系统级通知配置按开启口径发布，`env_of` 应保持 `TASK_FOLLOW_SCHEDULER_ENABLED=true`、`TASK_FOLLOW_NOTIFICATION_DELIVERY_MODE=all`、`TASK_FOLLOW_NOTIFICATION_ALLOWLIST_EMAILS=`。个人待归类事项通知开关仍默认关闭，需要责任人自行开启。
...
- 明确系统内生成个人周报可行，飞书原生汇报自动提交不进入本系统建设范围。
...
- 本版不新增 `weekly_reports` 表，不做确认、复制、导出、历史快照、飞书原生汇报回写和通知。
...
- 本版不新增数据库结构，不做导出文档、飞书原生汇报回写、通知恢复、责任人确认/退回、任务挂接、双审核、部门统计或会议处理。
...
- 拆分 4.4.1 至 4.4.6：责任人处理底座、挂载已有部门任务处理、本部门常态化处理、跨部门协作双确认、历史任务缺口延展、组合工作台复核。
- 该阶段曾规划 4.6.x 进入管理员运营工作台；4.6.0 已回正为工作台入口化与一屏化，管理员运营工作台暂缓。
...
- 版本推进到 `4.4.5`。
- 该版曾新增任务缺口延展状态、接口和视图。
- 4.5.3 复核 `4.2.1task.md` 后确认该能力超出当前系统主线，已从公开接口、前端入口和后续路线中下线。
- 历史字段仅作兼容保留，不再产生新数据。
...
- 版本推进到 `4.4.6`。
- 工作台主文案从单一执行者入口收口为个人行动入口，待归类事项区按多角色叠加展示“我的提交 / 待我处理 / 本部门常态化”。
- 待归类事项卡片优化长文本换行、可选处理字段展示和已转子任务提示，减少空字段噪音。
- 4.5.3 已下线历史任务缺口视图，回到四类待归类事项主线。
...
- 明确 4.5.x 回到 `4.2.1task.md` 的四类待归类事项和周报中心主线。
- 拆分 4.5.1 至 4.5.6：部门负责人工作台框架、部门任务补充闭环、路线回正与冗余下线、部门统计与周报材料视图、周报中心输出体验增强、4.5 收尾与 4.6 交接。
- 4.5.3 已确认不继续建设任务缺口对象、会议处理流或任务补建闭环；超出源文件的新对象必须先确认。
- 明确 4.5.x 不恢复生产通知、不做飞书原生汇报回写、不做正式会议纪要系统、不进入管理员运营工作台深度能力。

4.5.1 部门负责人工作台框架实施内容：

- 版本推进到 `4.5.1`。
- 新增只读接口 `GET /api/workbench/department-owner`，仅部门负责人可查看本人所属部门范围内的工作台聚合数据；无所属部门返回空态。
- 聚合范围包括本部门牵头母任务、本部门相关部门任务、待拆部门任务、本周未更新子任务、开放/处理中的风险、逾期子任务、本部门常态化记录和部门任务补充记录。
- 前端工作台新增“部门负责人”模块，按多身份叠加展示，不覆盖执行者任务、待归类事项和周报中心入口。
- 所有卡片只提供跳转到现有母任务、部门任务、子任务执行、会议看板或待归类事项区，不新增处理按钮。
- 本版不创建任务、不新增数据库结构、不恢复生产通知、不制作生产离线包。

4.5.2 部门任务补充闭环实施内容：

- 版本推进到 `4.5.2`。
- 按 `4.2.1task.md` 纠偏：本版主线是“待归类事项 -> 部门任务补充记录 -> 周报中心/任务追溯”。
- 后端为部门任务树增加 `work_item_supplements`，并提供单部门任务补充记录只读接口，方便部门任务页面和后续详情页追溯。
- 部门负责人工作台增加本部门范围内的“部门任务补充记录”汇总和列表。
- 部门任务页面展开区增加补充记录卡片，展示事项内容、提交人、状态、提交时间和关联子任务。
- 继续复用既有同意、退回、关闭和转子任务能力；本版不新增数据库结构、不创建部门任务或母任务、不恢复生产通知、不制作生产离线包。

4.5.3 待归类事项路线回正与冗余功能下线实施内容：

- 版本推进到 `4.5.3`。
- 以 `4.2.1task.md` 为最高指导口径，待归类事项公开能力回到四类对象：部门任务补充、本部门常态化、跨部门协作、周报补充。
- 下线此前延展出的任务缺口上提接口、scope、前端按钮、弹窗、工作台指标和分区；历史字段只做兼容保留，不删除数据库列。
- 部门负责人工作台移除任务缺口指标，只展示任务推进、部门任务补充、常态化、未更新、风险和逾期。
- 方案文档、路线图、验收清单和决策记录均同步回正；后续新增对象或流程必须先确认。

4.5.4 部门统计与周报材料视图实施内容：

- 版本推进到 `4.5.4`。
- 扩展部门负责人工作台接口，支持按周返回四类待归类事项的部门周报材料统计。
- 统计范围只包含部门任务补充、本部门常态化、跨部门协作和周报补充；已撤回事项不计入统计。
- 跨部门协作在提交部门和协作部门双方部门负责人视图中可见，并区分本部门提交与协作到本部门。
- 前端部门负责人模块新增“本周周报材料统计”分区，展示状态分布、四类事项汇总和最近记录。
- 本版不新增数据库结构、不新增处理动作、不影响正式任务树和会议看板统计、不恢复生产通知、不制作生产离线包。

4.5.5 周报中心输出增强实施内容：

- 版本推进到 `4.5.5`。
- 新增周报草稿复制接口，复制当前草稿不会创建或覆盖历史周报快照。
- 周报确认、历史复制和草稿复制统一使用新文本格式，按人员、周次、正式任务、部门任务补充、跨部门协作、常态化、周报补充、风险卡点和下周计划组织。
- 周报中心前端区分“复制当前草稿”和“复制已确认版本”，避免把复制误解为确认。
- 手动复制弹窗改为多行文本区域，展示周次和字数，方便浏览器剪贴板受限时手动复制。
- 本版不做执行人导出文档；后续导出面向观察者/人事，按人员和周次生成绩效评判材料另行规划。
- 文档口径已明确系统不做飞书原生汇报回写。

4.5.6 4.5 收尾与 4.6 交接实施内容：

- 版本推进到 `4.5.6`。
- 工作台文案收口为按身份叠加的个人行动入口，明确执行者任务、待归类事项、部门负责人模块和周报材料互不覆盖。
- 待归类事项区继续只围绕部门任务补充、本部门常态化、跨部门协作和周报补充四类对象，已处理记录只读追溯，不显示无效操作按钮。
- 周报中心说明强化：复制当前草稿不会生成历史快照，只有确认周报才会生成或覆盖本人周报快照；系统不做文档导出或飞书原生汇报回写。
- 部门负责人模块收口为只读查看本部门任务、未更新、风险逾期、部门任务补充、本部门常态化和本周周报材料统计，不新增处理动作。
- `4.2.1task.md` 补充边界说明：周报中心用于查看、复制和形成系统内快照；导出能力如后续建设，面向观察者/人事按人员和周次另行规划。
- 4.5.x 主线闭合。4.6.0 重新确认后，4.6.x 不进入管理员运营工作台，而是优先做工作台入口化与一屏化。

4.6.0 工作台入口化与一屏化规划实施内容：

- 版本推进到 `4.6.0`。
- 本版只做规划和文档同步，不改业务代码、不新增数据库结构、不恢复生产通知、不制作生产离线包。
- 明确 4.6.x 不做管理员运营工作台，也不做总经办工作台；工作台只保留执行者、任务负责人、部门负责人三个模块。
- 工作台定位从信息堆叠页调整为一屏行动入口：只展示关键数量、最紧急 2-3 项和跳转按钮。
- 规划 4.6.1 至 4.6.5：一屏框架、执行者模块入口化、任务负责人模块入口化、部门负责人模块入口化、视觉与手机收尾。
- 明确详细信息回到子任务执行、部门任务、待归类事项、周报中心和会议看板等业务页面处理。

4.6.1 工作台一屏框架实施内容：

- 版本推进到 `4.6.1`。
- 工作台默认首屏改为顶部身份/周次区 + 执行者、任务负责人、部门负责人三张摘要卡。
- 新增 `frontend/src/features/workbench/RoleSummaryCards.tsx`，将三类角色摘要卡和通用卡片展示从页面主体中拆出，避免继续堆大 `Workbench.tsx`。
- 执行者卡展示待更新、草稿、临近截止和可登记风险；任务负责人卡展示待处理事项、可转子任务和负责子任务；部门负责人卡展示部门任务、待拆、未更新、风险逾期和本周材料。
- 完整待归类事项处理面板和部门负责人详细面板改为首屏下方折叠区，默认收起，保留原有处理和追溯能力。
- 本版不新增数据库结构、不恢复生产通知、不制作生产离线包。

4.6.2 执行者模块入口化实施内容：

- 版本推进到 `4.6.2`。
- 新增 `frontend/src/features/workbench/ExecutorWorkbenchCard.tsx`，执行者卡从通用角色卡中独立出来，便于后续单独打磨。
- 执行者卡固定展示待更新、草稿未提交、临近截止、可登记风险四个关键数。
- 重点任务最多展示 3 条，排序为草稿未提交、临近截止、待更新、可登记风险；同类按截止日期近到远。
- 重点任务卡展示子任务编号、任务名、所属部门任务、截止日期、本周状态和更新/查看/风险入口。
- 本版不新增后端接口、不新增数据库结构、不恢复生产通知、不制作生产离线包；任务负责人和部门负责人卡保持 4.6.1 状态。

4.6.3 任务负责人模块入口化实施内容：

- 版本推进到 `4.6.3`。
- 新增 `frontend/src/features/workbench/TaskOwnerWorkbenchCard.tsx`，任务负责人卡从通用角色卡中独立出来。
- 工作台复用现有 `/department-tasks/overview` 获取当前用户可拆解的部门任务摘要，用于统计负责部门任务和待拆子任务，避免仅可见部门任务误进入任务负责人卡。
- 任务负责人卡固定展示待处理事项、可转子任务、负责部门任务、待拆子任务四个关键数。
- 重点项最多展示 3 条，排序为可转子任务事项、其他待处理事项、可拆解部门任务、待跟进负责子任务。
- 本版不新增后端接口、不新增数据库结构、不恢复生产通知、不制作生产离线包；执行者卡保持 4.6.2 状态，部门负责人卡保持 4.6.1 状态。

4.6.4 部门负责人模块入口化实施内容：

- 版本推进到 `4.6.4`。
- 新增 `frontend/src/features/workbench/DepartmentOwnerWorkbenchCard.tsx`，部门负责人卡从通用角色卡中独立出来。
- 工作台复用现有 `/workbench/department-owner` 数据，用于统计部门任务、未更新、风险/逾期、补充记录和本周材料。
- 部门负责人卡固定展示相关部门任务、未更新、风险/逾期、部门任务补充、本周材料五个关键数。
- 重点项最多展示 3 条，排序为风险/逾期、未更新、待拆部门任务、部门任务补充记录、本周材料异常。
- “查看本周材料统计”会展开部门负责人详细面板并定位到材料统计区域；本版不新增后端接口、不新增数据库结构、不恢复生产通知、不制作生产离线包。

4.6.5 工作台视觉与手机收尾实施内容：

- 版本推进到 `4.6.5`。
- 三张角色卡统一标题层级、指标块密度、重点项间距、按钮排列、空状态和长文本换行。
- 手机端强化三张卡纵向排列、按钮全宽、状态标签换行和重点项标题截断，避免全局横向滚动。
- 折叠区文案收口为“完整处理面板：待归类事项”和“部门负责人详细面板：完整部门材料”。
- 4.6.x 工作台入口化闭合；本版不新增后端接口、不新增数据库结构、不恢复生产通知、不制作生产离线包。

4.6.6 工作台三入口重排实施内容：

- 版本推进到 `4.6.6`。
- 根据验收反馈，工作台首屏改为“待归类事项 / 子任务执行周更新 / 拆解入口”三张行动入口卡。
- 待归类事项成为第一张独立卡，始终展示提交和处理入口，并展示我的提交、待我处理、可转子任务和待确认轻量数量。
- 移除工作台标题下方说明和当前人员大卡，当前人员与统计周次压缩为轻量信息。
- 部门负责人详情退出首屏三卡区，继续保留下方折叠详细面板。
- 执行和拆解入口重点项收紧为最多 2 条，减少首屏高度，完整列表继续回业务页面处理。
- 卡片网格改为按实际可见数量自动填满行宽；本版不新增后端接口、不新增数据库结构、不恢复生产通知、不制作生产离线包。

4.7.0 观察者月度周报 Excel 导出实施内容：

- 版本推进到 `4.7.0`。
- 新增观察者只读导出接口 `/api/weekly-reports/monthly-export?month=YYYY-MM`，管理员和权限管理人员仅作为技术兜底可调用。
- 导出按月份生成 Excel，列为该月份内每个周一对应的 ISO 周；每个 active 用户占 4 行：任务线完成、归类项完成、下周计划和遗留事项。
- 数据采用已确认周报优先，未确认周次实时草稿兜底并在单元格中标记 `[未确认草稿]`。
- 工作台新增“观察者周报导出”轻量卡，仅 `observer` 角色可见；普通用户仍保持待归类事项、子任务执行周更新、拆解入口三卡布局。
- 本版不新增数据库结构、不恢复生产通知、不制作生产离线包，不做飞书原生汇报回写。

4.7.1 总经办使用指南更新与 App.tsx 拆分实施内容：

- 版本推进到 `4.7.1`。
- 新增 `frontend/src/features/guides/`，将 legacy、总经办、部门负责人、任务负责人、执行者、观察者的 Tour steps 拆成独立文件。
- 新增 guide registry、routing helper 和 `useGuideTour` hook，承接使用指南自动弹出、手动重看、菜单触发、完成/跳过和保存进度逻辑。
- `App.tsx` 只保留页面 refs、菜单点击触发、右上角使用指南按钮和最终 `Tour` 渲染接入，不再承载大段角色引导文案和状态机。
- 总经办/秘书系统框架指南更新为工作台行动入口、会议看板审阅入口；会议看板专项指南更新为总览异常、风险逾期、未更新、部门差异、下钻明细和会后跟进。
- `CURRENT_ONBOARDING_VERSION` 和所有 guide version 提升到 `2`，所有用户下次进入系统会重新看到一次对应指南；其他角色文案本版只迁移不改写。
- 本版不新增数据库结构、不新增业务功能、不恢复生产通知、不制作生产离线包。

4.7.2 部门负责人和任务负责人使用指南更新实施内容：

- 版本推进到 `4.7.2`。
- 部门负责人指南更新为当前工作台三入口、部门负责人详细面板、本部门相关部门任务、部门任务补充、本部门常态化和本周材料统计口径。
- 任务负责人指南更新为拆解入口、待处理事项、可转子任务、部门任务补充处理、正式子任务转换和周更新跟进口径。
- 本版只改 `departmentOwner` 和 `taskOwner` 指南文件，不改总经办、执行者、观察者和 legacy 文案。
- 应用版本升到 4.7.2，但 `CURRENT_ONBOARDING_VERSION` 和所有 guide version 继续保持 `2`，已完成 v2 指南的用户不会再次自动弹出。
- 本版不新增数据库结构、不新增业务功能、不恢复生产通知、不制作生产离线包。

4.7.3 执行者和观察者使用指南更新实施内容：

- 版本推进到 `4.7.3`。
- 执行者指南更新为工作台“子任务执行周更新”入口、子任务更新、待归类事项、周报中心、附件、风险登记和完成撤回边界口径。
- 观察者指南更新为只读审阅、工作台多身份入口、会议看板审阅顺序、任务追溯、历史证据和月度周报 Excel 导出口径。
- 本版只改 `executor` 和 `observer` 指南文件，不改总经办、部门负责人、任务负责人和 legacy 文案。
- 应用版本升到 4.7.3，但 `CURRENT_ONBOARDING_VERSION` 和所有 guide version 继续保持 `2`，已完成 v2 指南的用户不会再次自动弹出。
- 本版不新增数据库结构、不新增业务功能、不恢复生产通知、不制作生产离线包。

4.7.4 待归类事项自动审批与飞书通知实施内容：

- 版本推进到 `4.7.4`。
- 新增 `work_item_automation_settings` runtime schema 和模型，责任人可按四类待归类事项配置个人飞书通知和自动同意，默认全部关闭。
- 新增 `/api/work-items/automation-settings` 读写接口和 `/api/work-items/{id}/revoke-auto-approval` 撤销接口。
- 创建待归类事项时按责任人个人设置触发自动同意；跨部门协作按提交部门侧和协作部门侧分别自动或人工确认，双确认完成后才变为已确认。
- 创建事项时按个人通知设置发送 `work_item_review_required` 或 `work_item_auto_approved` 飞书卡片，继续复用通知记录、签名链接、点击追踪、去重和系统级投递规则。
- 前端在工作台待归类事项卡和完整处理面板增加“审批与通知设置”，已自动同意事项展示来源和撤销入口。
- 本版不新增业务对象，不自动转子任务，不恢复生产通知，不制作生产离线包。

4.8.0 周更新快照安全回填实施内容：

- 版本推进到 `4.8.0`。
- 新增 `scripts/backfill_weekly_updates_from_excel.py`，用于从仓库根目录的 `2026公司工作任务跟踪表 (1).xlsx` 回填 `2026-W24` 周更新。
- 回填脚本默认 dry-run，输出可写入、已存在跳过、子任务未匹配、执行人未匹配和空内容等报告；只有显式传入 `--apply` 才写入数据库。
- 写入口径为只补空不覆盖，严格匹配子任务和执行人；匹配失败不做负责人兜底。
- 本次只读取 `04_周更新进度` 的本周完成内容、下周工作计划和遗留事项，不导入 W23 的上周任务进度，不创建历史附件记录。
- 脚本只写 `weekly_updates` 和回填事件，不反写子任务状态、进度、任务树或会议看板统计；本版不新增数据库结构、不恢复生产通知、不制作生产离线包。

4.8.1 历史周更新临时手工补录实施内容：

- 版本推进到 `4.8.1`。
- 历史时间线新增临时补录提示和“补录/编辑”按钮，仅管理员或 `permission.manage` 用户可见。
- 新增 `POST /api/weekly-updates/manual-history-upsert`，支持按子任务、执行人和周次补录或覆盖历史周更新。
- 保存后固定写入 `submitted` 状态，覆盖前生成 `WeeklyUpdateRevision`，并写入 `manual_history_weekly_update_upsert` 任务事件。
- `/api/timeline/matrix` 临时返回执行人、周更新元数据和编辑权限，供历史时间线弹窗选择执行人并回填本周完成、下周计划、遗留事项、进度和需协调。
- 本功能只用于本轮历史数据修复；不导入历史附件、不发送飞书通知、不自动改变子任务当前状态，下个版本删除入口和接口。

4.8.2 历史补录入口下线与工作台/时间线修正实施内容：

- 版本推进到 `4.8.2`。
- 删除历史时间线补录/编辑按钮、临时提示、补录弹窗和 `POST /api/weekly-updates/manual-history-upsert`。
- `/api/timeline/matrix` 不再返回临时编辑权限和编辑元数据，继续只读返回历史完成内容、遗留事项、附件和执行人信息。
- 历史时间线桌面端在子任务标题行展示执行人，手机端在任务卡片中展示执行人。
- 工作台“拆解入口”收窄为进入部门任务的单一入口；待归类事项处理和子任务执行分别由独立入口卡承载。
- 已通过 4.8.1 写入的历史补录数据保留；本版不新增数据库结构、不恢复生产通知、不制作生产离线包。

5.0.0 生产同步包实施内容：

- 版本推进到 `5.0.0`。
- 本版作为 4.x 阶段生产同步包，包含 4.8.x 已补齐的历史周更新数据。
- 更新生产打包脚本，支持 `TASK_FOLLOW_INCLUDE_DB_DUMP=true` 时将当前 PostgreSQL 数据库导出为 `database/task_follow_5_0_0_prod_data_YYYYMMDD.dump` 并放入离线包。
- 生产包继续放入仓库 `release_packages/`，不放 `/private/tmp` 作为最终交付位置。
- 系统级通知配置恢复开启：调度开启、投递模式为全员、通知白名单为空；个人待归类事项通知设置仍默认关闭。
- 生产手册更新为 5.0.0，并补充包内数据库恢复命令。

5.0.1 总经办部门任务全量查看修正实施内容：

- 版本推进到 `5.0.1`。
- 修正 `can_view_department_directory`：具备 `dashboard.view_all` 的用户可在部门任务页获得部门目录切换和全公司部门任务列表。
- 保持 `can_access_department_task` 原有规则不变；普通任务负责人、部门负责人、执行者不因此获得额外任务可见范围。
- 本次同步澄清权限口径：`dashboard.view_department` 是历史部门汇总权限名，不是部门任务页查看全量或本部门全部任务的实际开关。

5.0.2 部门任务权限与本部门汇总分组修正实施内容：

- 版本推进到 `5.0.2`。
- 调整部门任务拆解权限：管理员、`permission.manage`、总经理/秘书管理口径、部门任务负责人和对应部门负责人可拆解部门任务。
- 任务负责人可查看本人所属部门的部门任务汇总，但只有本人直接负责的部门任务可拆解；子任务执行者不获得本部门汇总。
- 部门任务接口返回 `viewer_is_direct_owner`、`viewer_is_department_scope` 和 `viewer_relation_label`，用于前端分组展示。
- 部门任务页改为“我负责 / 本部门任务汇总 / 全公司查看”分组；工作台拆解入口继续只统计本人直接负责的部门任务。

5.0.3 执行者本部门任务只读查看实施内容：

- 版本推进到 `5.0.3`。
- 执行者可在部门任务页查看本人所属部门的部门任务汇总，用于了解本部门重点工作。
- 执行者不获得部门任务编辑、子任务拆解、风险管理或代填周更新权限；按钮仍由后端 `can_edit`、`can_split` 等字段控制。
- 工作台保持原三入口结构，拆解入口继续只统计本人直接负责的部门任务。

5.0.4 工作台任务负责人入口个人化修正实施内容：

- 版本推进到 `5.0.4`。
- 工作台“拆解入口”从当前用户 ID 和部门任务 `owner_ids` 双重判断本人是否负责人，只统计本人直接负责的部门任务。
- 管理员、总经理、观察者等全局可见身份仍可在部门任务页查看和处理对应任务，但不再把全公司部门任务计入个人工作台拆解入口。

5.1.0 手动周提醒与补充入口卡实施内容：

- 版本推进到 `5.1.0`。
- 通知页手动周提醒扩展为一键发送两类飞书卡片：有待更新子任务人员收到周更新汇总卡，无待更新任务的 active 用户收到周报补充入口卡。
- 新增 `weekly_report_entry` 通知类型，卡片跳转工作台，提示用户补充临时工作、本部门常态化、跨部门协作或周报补充记录。
- 周更新汇总卡和补充入口卡按周次和人员交叉去重；重复手动发送只计入跳过，不重复创建通知记录。
- 每周五定时周更新提醒保持原待更新子任务扫描口径，不自动发送无任务人员补充入口卡。
