...
公司任务跟踪系统是面向公司级任务推进、每周更新、风险协调和会议看板的独立 Web 系统。3.0.0 是生产第一版；当前 4.8.1 在历史时间线临时开放历史周更新手工补录，用于修正 4.8.0 自动回填不够完整或匹配错误的数据。补录入口仅管理员和 `permission.manage` 可见，保存后直接写入正式已提交周更新；该能力只用于开发环境数据修复，下个版本将删除入口和接口。生产通知仍保持暂停口径，本版不制作生产离线包，不做飞书原生汇报回写。
...
- 4.8.1 在历史时间线临时开放历史周更新手工补录；管理员或 `permission.manage` 可按子任务、执行人和周次补录/编辑本周完成、下周计划、遗留事项、进度和需协调，保存后作为正式已提交周更新落库。
...
4.8.1 针对自动回填不够完整或匹配错误的情况，临时在“历史时间线”开放手工补录。管理员或 `permission.manage` 用户可以在对应周次单元格点击“补录/编辑”，选择执行人并填写本周完成、下周计划、遗留事项、进度和需协调。保存会直接写入 `submitted` 周更新，并生成修订记录和任务事件；不会导入历史附件，不会发送飞书通知，也不会自动改变子任务当前状态。该能力只用于本轮历史数据修复，下个版本将删除。
